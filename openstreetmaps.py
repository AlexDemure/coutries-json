    ...
    def get(cls, proxy: Proxy, city: dict) -> dict:
        ...
        for language in Settings.AVAILABLE_LANGUAGES:
            ...
            if names.get(language):
                response = cls.request(proxy=proxy, params=dict(data=cls._template(city=names[language], language=language)))
                if response.get('elements'):
                    elements = elements + response['elements']

                response = cls.request(proxy=proxy, params=dict(data=cls._template(city=names[language])))
                if response.get('elements'):
                    elements = elements + response['elements']

            response = cls.request(proxy=proxy, params=dict(data=cls._template(city=names['ascii'], language=language)))
            if response.get('elements'):
                elements = elements + response['elements']

            response = cls.request(proxy=proxy, params=dict(data=cls._template(city=names['origin'], language=language)))
            if response.get('elements'):
                elements = elements + response['elements']

        response = cls.request(proxy=proxy, params=dict(data=cls._template(city=names['ascii'])))
        if response and response.get('elements'):
            elements = elements + response['elements']

        response = cls.request(proxy=proxy, params=dict(data=cls._template(city=names['origin'])))
        if response and response.get('elements'):
            elements = elements + response['elements']
        ...
        for element in elements:
            for language in Settings.AVAILABLE_LANGUAGES:
                if element['tags'].get(f'name:{language}'):
                    if language == 'en':
                        tags[language].append(unidecode(element['tags'][f'name:{language}']))
                    else:
